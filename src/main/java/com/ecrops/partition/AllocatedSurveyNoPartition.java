...
public class AllocatedSurveyNoPartition {
	...
	public  List<AllocatedSurveyNo> getAllSno(String wbdcode, String mcode, String cropyear) {
		...
		if (seasonYear >2022  && !(seasonYear==2023 && seasonType.equalsIgnoreCase("S"))) {
		...
		String Sql = "select wbevname  as lgdvname,cast(coalesce(data_src,' ') as varchar) as data_src,cast(kh_no as varchar) as kh_no ,cr_sno,coalesce(tot_extent,0.00) as tot_extent,coalesce(geo_reffered ,'NO') as geo_reffered \r\n"
		...
		for (Object[] row : detailsEntities1) {
			...
			String datasrc = (String) row[1].toString();
		 ...
		 entity.setData_src(datasrc);
			...
			entity.setCr_sno((String) row[3].toString());//System.out.println("3"+row[3]);
			    entity.setTot_extent((BigDecimal) row[4]);
			
			    if (row[5].toString().equals("Y")) {
			        entity.setGeo_reffered("Yes");
			    } 
			    else{
