...
public class SaveSocialAudit {
	...
	@Transactional
	public String saveSocialAuditDetails(String[] bookingIds, String[] kh_no, String[] sr_no, String[] sowDt,
			String[] cropName, String[] varietyName, String[] totExtent, String[] farmername, String[] farmerFather,
			String[] aadharId, String[] waterId, String[] cropNatureId, String[] farmingType, String[] bookingIds_org,
			String[] cr_no_org, String[] kh_no_org, String[] sr_no_org, String[] sowDt_org, String[] cropName_org,
			String[] varietyName_org, String[] totExtent_org, String[] farmername_org, String[] farmerFather_org,
			String[] aadharId_org, String[] waterId_org, String[] cropNatureId_org, String[] farmingType_org,
			String[] action_type, String[] del_list, String applName, String appMobId, String applicatonId,
			String clientip, String season, String cropyear, Integer wbldcode, Integer mandcode, Integer villcode,
			String activeYear, String[] areaSownList) {
		System.out.println("areaSownList---->" + Arrays.toString(areaSownList));
		System.out.println("totExtent_org---->" + Arrays.toString(totExtent_org));
		...
		System.out.println("farmername_org---->" + Arrays.toString(farmername_org) + "farmerFather_org -----> "
				+ Arrays.toString(farmerFather_org));
		System.out.println("farmername---->" + Arrays.toString(farmername) + "farmerFather -----> "
				+ Arrays.toString(farmerFather));
		...
		String qry4 = "select * from " + farmerGrievTab
				+ " where cr_dist_code=? and bookingid=? and cr_crop=? and cr_no=? and cr_vcode=? and variety=? and kh_no=? and cr_sow_date=?";
		...
		for (int i = 0; i < bookingIds_org.length; i++) {
			...
			if (!resultList.isEmpty()) {
				String qry5 = "update " + farmerGrievTab
						+ " set mro_flag='V',variety_sug=?,farmername_sug=?,fathername_sug=?,extent_sug=?,cr_w_draw_sug=?,cr_crop_sug=?,cropschtype_sug=?,cr_sow_date_sug=?,resubmitbyva='Y',"
				...
				if (areaSownList[i].trim().isEmpty()) {
					ps1.setParameter(4, 0);
				} else {
					ps1.setParameter(4, Double.valueOf(areaSownList[i]));
				}
				if (waterId[i].trim().isEmpty()) {
					ps1.setParameter(5, 0);
				} else {
					ps1.setParameter(5, Integer.parseInt(waterId[i].trim()));
				}
				...
				if (sowDt[i].trim().isEmpty()) {
					ps1.setParameter(8, Date.valueOf("2024-01-01"));
				} else {
				...
				if (action_type[i].equals("del")) {
					...
					String qry2 = "update " + partitionName
							+ " set mark_delet='Y', social_status=?, reason_non_auth=? where cr_dist_code=? and bookingid=? and cr_crop=? and cr_no=? and cr_vcode=? and variety=? and cr_sow_date=?";
				...
				String qry1 = "insert into " + farmerGrievTab
						+ "(cr_dist_code, cr_mand_code, cr_vcode,  bookingid, cr_sno, kh_no, "
				...
				if (areaSownList[i].trim().isEmpty()) {
					...
					pst.setParameter(17, Double.valueOf(areaSownList[i]));
				}
				if (waterId[i].trim().isEmpty()) {
					pst.setParameter(18, 0);
				} else {
					pst.setParameter(18, Integer.parseInt(waterId[i].trim()));
				}
				if (sowDt[i].trim().isEmpty()) {
					System.out.println("date11----->" + sowDt[i]);
					pst.setParameter(19, Date.valueOf("2024-01-01"));
				} else {
					System.out.println("date----->" + sowDt[i]);
				...
				pst.setParameter(30, Integer.parseInt(cropyear));
				pst.setParameter(31, season);
				if (action_type[i].equals("del")) {
				...
				String qry = "select grievance_id from " + farmerGrievTab
						+ " where cr_dist_code=? and bookingid=? and cr_crop=? and cr_no=? and cr_vcode=? and variety=? and kh_no=? and cr_sow_date=?";
				...
				try {
				    greivanceId = (Integer) ps1.getSingleResult(); // Assuming grievance_id is an Integer
				    System.out.println("grievance id--------->" + greivanceId);
				    // Now you have the grievance_id in grievanceId variable
				} catch (NoResultException e) {
				    System.out.println("No grievance found for the given parameters.");
				} catch (NonUniqueResultException e) {
				    System.out.println("More than one grievance found for the given parameters.");
				} catch (Exception e) {
				    e.printStackTrace(); // Handle other exceptions appropriately
				}
//				System.out.println("griev qry--->"+qry);
//				if (object.size() > 0) {
//					Object[] grievanceIdArray = object.get(0);
//					Integer grievanceId = (Integer) grievanceIdArray[0];
//					System.out.println("grievance id--------->"+grievanceId);
//				}

				String qry3 = "INSERT INTO " + socialAuditRejTab
						...
						+ "supercheck_appr, extent_check, oldextent, uidmultiple," + greivanceId
						+ ", social_status from " + partitionName
						+ " where cr_dist_code=? and bookingid=? and cr_crop=? and cr_no=? and cr_vcode=? and variety=? and kh_no=? and cr_sow_date=?";
				...
				String griev_comp = "Y";
				String qry2 = "update " + partitionName
						+ " set griev_comp=?,grievance_id=? where cr_dist_code=? and bookingid=? and cr_crop=? and cr_no=? and cr_vcode=? and variety=? and cr_sow_date=?";
				ps1 = entityManager.createNativeQuery(qry2);
