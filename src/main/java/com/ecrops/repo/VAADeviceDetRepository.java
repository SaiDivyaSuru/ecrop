...
//			+ "ORDER BY blockortehsil,village",nativeQuery=true)
	@Query(value = "SELECT DISTINCT district, mname,vname,blockortehsil,village,userid,\r\n"
			...
			+ "ORDER BY mname,village,dt_crt",nativeQuery=true)
	...
	@Query(value = "select cropid as col1,coalesce(cropnameeng,'NA') as col2 from cropnames  where active='A' order by cropnameeng", nativeQuery = true)
