...
//					+ efish_tab + ") " + " and  cr_vcode=?  and vs_sel='Y' and cr_sno=? and kh_no=? and rec_id=? ";
			...
			for (int i = 0; i < vcodes.length; i++) {
				...
				System.out.println("season------------>"+season);
				System.out.println("Integer.parseInt(vcodes[i].trim())------------>"+Integer.parseInt(vcodes[i].trim()));
				System.out.println("Long.parseLong(regNos[i].trim())------------>"+Long.parseLong(regNos[i].trim()));
				System.out.println(" Long.parseLong(sjoints[i].trim())------------>"+ Long.parseLong(sjoints[i].trim()));
				System.out.println("surveyNo[i].trim()------------>"+surveyNo[i].trim());
				System.out.println("Long.parseLong(khno[i].trim())------------>"+Long.parseLong(khno[i].trim()));
				System.out.println("Long.parseLong(recIds[i])------------>"+Long.parseLong(recIds[i]));
...
//				}
				...
				status = crbIns;
				System.out.println("rbkupd----------->"+rbkupd);
				System.out.println("crbIns----------->"+crbIns);
				System.out.println("crbUpd----------->"+crbUpd);
//				if (rbkupd > 0 && crbIns >= 0 && crbUpd > 0 && downdetIns >= 0) {
	...
	public List<MarketPicePojo> listMarketPrice(String apmc, String commidity, String fromDate, String toDate) {

		String var = "SELECT pid, state, mandi, commodity, modal_price, traded_quantity, quantity_uom, maxprice,  soldqty, minprice, arrival_quantity, tran_date, date\r\n"
				+ "	FROM ecrop2024.market_prices  \r\n" + "	where mandi='" + apmc + "' and commodity='" + commidity
				+ "' and tran_date between '" + fromDate + "' and '" + toDate + "' ";

		List<MarketPicePojo> pojo = new ArrayList<>();
		...
		if (objects != null && objects.size() > 0) {
			...
			for (Object eol : objects) {
				...
				MarketPicePojo pojos = new MarketPicePojo();
				pojos.setPid(Integer.parseInt(row[0].toString()));
				pojos.setState(row[1].toString());

				pojos.setMandi(row[2].toString());
				pojos.setCommodity(row[3].toString());
				pojos.setModalPrice((BigDecimal) row[4]);

				pojos.setTradedQuantity((BigDecimal) row[5]);
				pojos.setQuantityUom(row[6].toString());
				pojos.setMaxPrice((BigDecimal) row[7]);
				pojos.setSoldQuantity((BigDecimal) row[8]);

				pojos.setMinPrice((BigDecimal) row[9]);

				pojos.setArrivalQuantity((BigDecimal) row[10]);

				pojos.setTranDate((Date) row[11]);
				pojos.setDates((Date) row[12]);
				pojo.add(pojos);
			}
		}
	...
	public List<FarmerAlertsTop> getFarmerAlerts() {
		String var = "SELECT message, dt_create\r\n"
				+ "	FROM advisories.farmer_alerts_top5_v;";

		List<FarmerAlertsTop> pojo = new ArrayList<>();
		...
		if (objects != null && objects.size() > 0) {
			...
			for (Object eol : objects) {
				...
				FarmerAlertsTop pojos = new FarmerAlertsTop();
				pojos.setMessage(row[0].toString());
				pojos.setMsgDate(row[1].toString());
	...
	public List<AdvisoriesSent> getAdvisoriesDateWise(int district,int crop, String fromDate,String toDate) {
//		String var = "SELECT district, crop, dt_sent, temp_content, total_sent\r\n"
//				+ "	FROM advisories.advisories_sent_date_wise_v  where cr_dist_code='"+district+"' and crop='"+crop+"' and dt_sent between '"+fromDate+"' and '"+toDate+"';";
//
//		
//		
		String var = "SELECT district, crop, dt_sent, temp_content, total_sent\r\n"
				+ "	FROM advisories.advisories_sent_date_wise_v  where cr_dist_code="+district+" and cropid="+crop+" and dt_sent between '"+fromDate+"' and '"+toDate+"';";

		List<AdvisoriesSent> pojo = new ArrayList<>();
		...
		if (objects != null && objects.size() > 0) {
			...
			for (Object eol : objects) {
				...
				AdvisoriesSent pojos = new AdvisoriesSent();
				pojos.setMessage(row[3].toString());
				pojos.setDateSent(row[2].toString());
				pojos.setCrop(row[1].toString());
				pojos.setDistrict(row[0].toString());
				pojos.setTotalSmsSent(row[4].toString());
	...
	public List<DistrictsPojo> getDistrictsList() {
		String var = "select wbdcode,dname from district_2011_cs where wbdcode<>88 order by dname";

		List<DistrictsPojo> pojo = new ArrayList<>();
		...
		if (objects != null && objects.size() > 0) {
			...
			for (Object eol : objects) {
				...
				DistrictsPojo pojos = new DistrictsPojo();
				pojos.setWbdcode(Integer.parseInt(row[0].toString()));
				pojos.setDistrict(row[1].toString());
	...
	public List<CropPojo> getCropList() {
		String var = "select cropid,cropnameeng from cropnames where active='A' order by cropname";

		List<CropPojo> pojo = new ArrayList<>();
		...
		if (objects != null && objects.size() > 0) {
			...
			for (Object eol : objects) {
				...
				CropPojo pojos = new CropPojo();
				pojos.setCropId(Integer.parseInt(row[0].toString()));
				pojos.setCropname(row[1].toString());
	...
	public List<FarmerAlertsTop> getnewsAlertt() {
		String var = "SELECT news, dt_crt	FROM advisories.agri_webmsg where status='A' order by dt_crt";
