...
public interface wholeSurveyNoMandWiseRepository extends JpaRepository<wholeSurveyNoMandWise, String>{
	@Query(value="select wbevname as lgdvname,cr_wsno from (select * from masters.wholesurveynolist \r\n"
			+ "where wbdcode=:wbdcode and wbmcode=:wbmcodenew) a,(select * from wbvillage_mst \r\n"
			+ "where wbdcode=:wbdcode and wbmcode=:wbmcodenew) b where a.wbvcode=b.wbvcode  order by wbevname ",nativeQuery=true)
