...
//					}
					...
					model.addAttribute("regDetails", regDet);
					msg="Selected Employee Is Already a Regular";
				...
				String status = "N";
				String sts =null,rbkCheck =null,empCheck= null;
				Long countttt = employeeService.checkInchargeExt(Integer.parseInt(mandal), Integer.parseInt(rbkuserCode) ,  Integer.parseInt(emp));	
				try {
					empCheck = employeeService.checkEmpStsA(Integer.parseInt(mandal),  Integer.parseInt(emp));
				}
				catch(Exception e) {
					e.printStackTrace();
				}
				if (empCheck == null) {
					msg="The Employee need to be Regular for any RBK";
					model.addAttribute("msg", msg);
					List<EmployeeName> EmpName = empRepo.findEmpName(Integer.parseInt(mandal));
					List<VillageName> villname = villRepo.findVillName(district, mandal);
					model.addAttribute("EmpName", EmpName);
				...
				try {
					rbkCheck =  employeeService.checkInchargeStsA(Integer.parseInt(mandal), Integer.parseInt(rbkuserCode) );//check of both// check of incharge
				}
				catch(Exception e) {
					e.printStackTrace();
				}
//				
			try {
				sts =  employeeService.checkInchargeStsA(Integer.parseInt(mandal), Integer.parseInt(rbkuserCode) ,  Integer.parseInt(emp));
				if(sts.equalsIgnoreCase("R")) {
					List<EmployeeName> EmpName = empRepo.findEmpName(Integer.parseInt(mandal));
					List<VillageName> villname = villRepo.findVillName(district, mandal);
					model.addAttribute("EmpName", EmpName);
					model.addAttribute("villname", villname);
					...
					System.out.println("regDet------------------------------------------------->" + regDet.size());
					model.addAttribute("regDetails", regDet);
					msg="Selected Employee is Already Regular for the same RBK";
					model.addAttribute("msg", msg);
					return "maoroles/rbkmapping";
...
//			else if(countttt== 0 && empCheck!= null){
...
//			}
			try {
				rbkCheck =  employeeService.checkInchargeStsA(Integer.parseInt(mandal), Integer.parseInt(rbkuserCode) );
			}
			catch(Exception e) {
				e.printStackTrace();
			...
			/*System.out.println("rbkCheck-----------"+rbkCheck);
			if(rbkCheck == null || rbkCheck.isEmpty()) {
				msg="There is no Regular for this RBK. So please, select Regular First";
				model.addAttribute("msg", msg);
				List<EmployeeName> EmpName = empRepo.findEmpName(Integer.parseInt(mandal));
				List<VillageName> villname = villRepo.findVillName(district, mandal);
				model.addAttribute("EmpName", EmpName);
