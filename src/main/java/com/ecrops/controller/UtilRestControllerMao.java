...
import com.ecrops.partition.ResurveyStatusDMVPartition;
	...
	public List<FarmerDetails> farmerdet(@RequestBody RequestModel requestModel) {
		...
		if (wbdcd && wbmcd ) {
			...
			List<FarmerDetails> farmerDetails = farmerDetailsPartition.farmerDetails(
					...
					requestModel.getDate(),requestModel.getVcode());
		...
		if (dcode && mcode && year) {
			...
			try {
				...
				List<NonWebView> nonwebv = nonWebViewPartition.getNonwebView(
						requestModel.getDcode(), 
						requestModel.getMcode(), 
						requestModel.getCropyear(),requestModel.getVcode());
...
//		boolean saesonid = regexpressionmethod.year(seasonYear.toString());
	...
	public List<AllocatedSurveyNo> getAllSnumber(@RequestBody RequestModel requestModel) {
		...
		if (mcode && saesonid) {
			...
			try {
				...
				System.out.println("---------------->"+allsno.get(0).getData_src());
				System.out.println("---------------->"+allsno.get(0).getGeo_reffered());
				return allsno;
			} catch (Exception e) {
	...
	//======================================================Resurveyed Status=======================================//
		...
		@Autowired
		ResurveyStatusDMVPartition resurveyStatusDMVPartition;

		@PostMapping("reserveydS")
		List<ResurveyStatusDMV> getResurveydS(@RequestBody RequestModel requestModel) {
			System.out.println("requestModel=>"+requestModel.toString());
			List<ResurveyStatusDMV> rst = resurveyStatusDMVPartition.getResurveyStatus(requestModel.getRole(),
					requestModel.getWbdcode(),
					requestModel.getWbmcode());
